...
public class RequestKitBean {
    ...
    public static final String REQ_CONTEXT_KEY_PARAMERJSON = "REQ_CONTEXT_KEY_PARAMERJSON";

    /**
     * JSON格式通过请求主体BODY传输，获取参数
     */
    ...
    private boolean isConvertJSON() {
        ...
        if (contentType != null && contentType.toLowerCase().indexOf("application/json") >= 0 && !request.getMethod().equalsIgnoreCase("GET")) {
    ...
    /**
     * request.getParameter 获取参数，并转换成为JSON格式
     *
     * @return
     */
    ...
    public JSONObject getReqParamJSON() {
        ...
        Object reqParamObject = RequestContextHolder.getRequestAttributes().getAttribute(REQ_CONTEXT_KEY_PARAMERJSON, RequestAttributes.SCOPE_REQUEST);
        if (reqParamObject == null) {
            JSONObject reqParam = reqParam2JSON();
            RequestContextHolder.getRequestAttributes().setAttribute(REQ_CONTEXT_KEY_PARAMERJSON, reqParam, RequestAttributes.SCOPE_REQUEST);
            return reqParam;
        }
