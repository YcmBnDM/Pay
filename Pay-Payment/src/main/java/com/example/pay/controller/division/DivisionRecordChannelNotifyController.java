...
import com.example.pay.channel.AbstractDivisionRecordChannelNotifyService;
...
public class DivisionRecordChannelNotifyController extends AbstractCtrl {
    ...
    public ResponseEntity doNotify(HttpServletRequest request, @PathVariable("ifCode") String ifCode) {
        ...
        try {
            ...
            List<PayOrderDivisionRecord> recordList = payOrderDivisionRecordService.list(PayOrderDivisionRecord.gw().eq(PayOrderDivisionRecord::getState, PayOrderDivisionRecord.STATE_ACCEPT).eq(PayOrderDivisionRecord::getBatchOrderId, divisionBatchId).orderByAsc(PayOrderDivisionRecord::getRecordId));
