...
import com.example.pay.channel.IPaymentService;
...
import com.example.pay.rqrs.req.payorder.payway.cashier.QrCashierOrderRQ;
import com.example.pay.rqrs.resp.payorder.UnifiedOrderRS;
import com.example.pay.rqrs.resp.payorder.payway.cashier.QrCashierOrderRS;
import com.example.pay.service.ConfigContextQueryService;
import com.example.pay.service.PayOrderProcessService;
...
public abstract class AbstractPayOrderController extends ApiController {
    ...
    protected ApiRes unifiedOrder(String wayCode, UnifiedOrderRQ bizRQ, PayOrder payOrder) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(bizRQ.getNotifyUrl()) && !StringUtil.isAvailableUrl(bizRQ.getNotifyUrl())) {
                throw new BizException("异步通知地址协议仅支持http:// 或 https:// !");
            }
            if (StringUtils.isNotEmpty(bizRQ.getReturnUrl()) && !StringUtil.isAvailableUrl(bizRQ.getReturnUrl())) {
    ...
    private PayOrder genPayOrder(UnifiedOrderRQ rq, MchInfo mchInfo, MchApp mchApp, String ifCode, MchPayPassage mchPayPassage) {
        ...
        payOrder.setPayOrderId(SequenceUtil.genPayOrderId()); //生成订单ID
