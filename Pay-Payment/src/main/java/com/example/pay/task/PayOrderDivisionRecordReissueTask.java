...
import com.example.pay.channel.IDivisionService;
...
public class PayOrderDivisionRecordReissueTask {
    ...
    public void start() {
        ...
        LambdaQueryWrapper<PayOrderDivisionRecord> lambdaQueryWrapper = PayOrderDivisionRecord.gw().eq(PayOrderDivisionRecord::getState, PayOrderDivisionRecord.STATE_ACCEPT).le(PayOrderDivisionRecord::getCreatedAt, offsetDate);
            ...
            try {
                ...
                for (PayOrderDivisionRecord batchRecord : pageRecordList.getRecords()) {
                    ...
                    try {
                        ...
                        List<PayOrderDivisionRecord> recordList = payOrderDivisionRecordService.list(PayOrderDivisionRecord.gw().eq(PayOrderDivisionRecord::getState, PayOrderDivisionRecord.STATE_ACCEPT).eq(PayOrderDivisionRecord::getBatchOrderId, batchOrderId).orderByAsc(PayOrderDivisionRecord::getRecordId));
                        ...
                        recordList.stream().forEach(record -> {
                            ...
                            if (ChannelRetMsg.ChannelState.CONFIRM_SUCCESS == channelRetMsg.getChannelState() || ChannelRetMsg.ChannelState.CONFIRM_FAIL == channelRetMsg.getChannelState()) {
