...
public class AuthService {
    ...
    public String auth(String username, String password) {
        ...
        PayUserDetails payUserDetails = (PayUserDetails) authentication.getPrincipal();

        //验证通过后 再查询用户角色和权限信息集合

        SysUser sysUser = payUserDetails.getSysUser();
        ...
        // 放置权限集合
        payUserDetails.setAuthorities(getUserAuthority(sysUser));

        //生成token
        String cacheKey = CS.getCacheKeyToken(sysUser.getSysUserId(), IdUtil.fastUUID());

        //生成iToken 并放置到缓存
        TokenService.processTokenCache(payUserDetails, cacheKey); //处理token 缓存信息

        //将信息放置到Spring-security context中
        UsernamePasswordAuthenticationToken authenticationRest = new UsernamePasswordAuthenticationToken(payUserDetails, null, payUserDetails.getAuthorities());
        SecurityContextHolder.getContext().setAuthentication(authenticationRest);

        //返回JWTToken
        return JWTUtil.generateToken(new JWTPayload(payUserDetails), systemYmlConfig.getJwtSecret());
    }
